---
name: tokenspeed
description: 查询 AI 编码工具的模型生成速度统计（tokens/s、首 token 延迟 TTFT、输出 token 量），支持 ZCode、Codex、OpenCode、Claude Code。当用户询问生成速度多快、tokens per second、tok/s、速度统计、模型输出快不快等与模型输出速度相关的问题时使用。
---
...
- 用户指定看某个工具时追加 `--tool`：`zc`=ZCode、`cx`=Codex、`oc`=OpenCode、`cc`=Claude Code。
