---
description: 查询模型生成速度统计（tokens/s、TTFT、输出 token 量），支持 ZCode/Codex/OpenCode/Claude Code/Pi
argument-hint: "[次数] [工具: zc|cx|oc|cc|pi]"
---
...
- 若用户只想看某个工具，追加 `--tool` 参数：`zc`=ZCode、`cx`=Codex、`oc`=OpenCode、`cc`=Claude Code、`pi`=Pi（如 `/tokenspeed codex` → `--tool cx`）。
...
2. 程序 stdout 包含：各工具最近一次速度、ZCode 会话汇总、最近若干次明细表（SRC 列标识来源工具：ZC=Codex 前的 ZCode、CX=Codex、OC=OpenCode、CC=Claude Code、PI=Pi）。把它们原样或稍作排版回复给用户，不要编造或修改数字。
